...
- ✅ **MOBİL BİRLEŞTİRİLDİ (18.08.2026):** public/mobil/ PWA yakalama arkadaşı + lib/mobil.js. QR eşleştirme (masaüstünde 📱 Telefon düğmesi), AES-CTR+HMAC şifreli LAN senkron, telefon "gelen kutusu" (kitaba doğrudan yazmaz; masaüstü birleştirmeden önce otomatik versiyon alır). Güvenlik: tüm /api ve /exports yalnız localhost; LAN'a yalnız /mobil sayfası + şifreli /api/mobile/sync açık. 70 yeni i18n anahtarı × 10 dil. Bağımlılıklar: aes-js, js-sha256, qrcode. Saha testi (gerçek telefon + Wi-Fi) kullanıcıda.
- ✅ **Ölçüm altyapısı (20.08.2026):** Siteye çerezsiz GoatCounter sayacı (`tools/build-site.mjs` → `GOATCOUNTER` sabiti; boş bırakılırsa hiç script üretilmez) · indirme düğmelerinde `data-goatcounter-click` olayları · `npm run stats` ile GitHub Releases indirme sayıları (`tools/download-stats.mjs`) · sitemap'e kaynak dosya tarihinden türeyen `lastmod` · gizlilik sayfasına 10 dilde "Site istatistik topluyor mu?" bölümü. Search Console/Bing doğrulaması **DNS TXT ile** yapılacak — build `public/site/`'ı sildiği için HTML doğrulama dosyası kalıcı olmaz. Adımlar: [docs/olcum-kurulumu.md](docs/olcum-kurulumu.md). Taban çizgisi: 3 indirme (v1.0.0).
- ✅ **Teknik SEO (21.08.2026):** Yapısal veri — 20 sayfada `SoftwareApplication` (ücretsiz teklif, OS, sürüm package.json'dan, indirme bağlantısı), 10 gizlilik sayfasında `FAQPage` (dört soru; JSON-LD'ye düz metin olarak girer). Uydurma puan/yorum eklenmedi. Sosyal paylaşım: 1200×630 `og.png` (kaynağı `site-src/og-image.html`, dile bağlı metin taşımaz), `twitter:card` → `summary_large_image`, `og:image:alt` 10 dilde, `og:locale:alternate` × 9. Doğrulama: 40 sayfa, 30 JSON-LD bloğu, geçersiz yok.
- ✅ **Rehber web'de (21.08.2026):** `/guide.html` + `/<dil>/guide.html` — 10 yeni sayfa, site 40 → **50 URL**. Gövde `public/guide/<dil>.json`'dan gelir (uygulamanın rehberiyle tek kaynak); `guideBody()` uygulama düğmelerini temizler, h1'i `guide_og_title` ile değiştirir, kalıntı kalırsa build'i durdurur. Dil başına ~1200 kelime (TR); `Article` JSON-LD, hreflang, sitemap, header+footer bağlantısı. **Bölümlere ayrılmadı:** rehber toplam ~900-1200 kelime, 6-7 bölüme bölünse her biri ~150 kelimelik ince içerik olurdu.
- ✅ **Destek zinciri canlı:** site destek kartları Shopier ürünlerine bağlı (49993606/49993994/49994027) · **Pazarlama sistemi:** marketing/ içerik kiti + haftalık bulut rutini (Pzt 09:23) + GitHub Discussions + ana sayfada 10 dilli geri bildirim bölümü
- ✅ **Veri evi taşındı + kayıp kitap kurtarma (24.09.2026):** Sahadan gelen ilk ciddi kullanıcı kaybı — exe yeniden indirilince uygulama boş açıldı, kitap eski klasörde kaldı. Üç ayrı kusur vardı: (1) paketli sürüm veriyi exe'nin YANINA yazıyordu, exe ise çoğu kez İndirilenler'de kalıyor; (2) kurulum sihirbazı `localStorage`'daki tek `onboarded` işaretine bakıyordu — işaret exe'ye değil localhost adresine bağlı olduğu için yeniden indiren kullanıcıda kalıyor ve "verileriniz şurada" ekranı tam gerektiği anda açılmıyordu; (3) sihirbaz mutlak yolu hiç göstermiyordu, yalnızca `data/` yazıyordu.
  **Çözüm:** Veri evi artık `Belgeler/inkGuide` (`server.js` → `resolveHome()`). Geriye uyumluluk üç istisnayla korundu: `INKGUIDE_HOME` ortam değişkeni · exe yanında `tasinabilir.txt` (USB modu) · exe yanında zaten bir `data/library.json` varsa orada kalır ve arayüz taşımayı **teklif eder**, kendiliğinden taşımaz. Taşıma kopyalar, sonra kaynağı `data-tasindi-<zaman>` olarak adlandırır — hiçbir dosya silinmez.
  Yeni uçlar: `GET /api/konum` (mutlak yollar + mod + `yeniKurulum`), `POST /api/konum/ac` (klasörü Explorer/Finder'da aç, beyaz listeli), `POST /api/konum/tasi`, `GET /api/kurtarma/tara` (İndirilenler/Masaüstü/Belgeler/OneDrive/exe klasörü, 2 alt klasör derinlik, 4000 klasör tavanı), `POST /api/kurtarma/aktar` (kopyalar; boş varsayılan kitabın üstüne yazar, dolu kitabın üstüne asla).
  Arayüz: tanıtım artık **veri evi başına** bir kez açılır (`onboardedFor:<yol>`); sihirbaz adımları sabit değil — taramada başka kitaplık bulunursa en başa "Kitabınızı bulduk" adımı eklenir; Ayarlar'daki veri kartı mutlak yolları, "Klasörü aç" düğmelerini, Belgeler'e taşıma uyarısını ve her an çalışan "Başka klasördeki kitabı bul" taramasını içerir. 21 yeni i18n anahtarı × 10 dil; rehbere "Kitaplarınız nerede durur?" bölümü (public/guide/<dil>.json → hem uygulama hem /guide.html); indirme sayfasına `dl_place` kutusu + yeni ilk adım (üç adım dörde çıktı).
  **Doğrulama:** paketli exe geçici bir "İndirilenler" klasöründen çalıştırıldı → veri `Belgeler/inkGuide`'a yazıldı, exe klasörüne dokunulmadı; tarama diskteki gerçek kitaplığı (Problem, 4806 kelime) buldu, içe aktardı, kaynak dosyanın boyutu ve tarihi değişmedi; eski kurulum taklidiyle `exe-yani` modu ve taşıma da uçtan uca denendi.
